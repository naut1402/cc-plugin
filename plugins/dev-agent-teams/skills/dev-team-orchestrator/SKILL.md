...
argument-hint: "[task-id] [--resume] [--subtask-of=<parent-id>] [--auto-review] [--export-json] [--remote] [--project=<id>] [--server=<url>]"
user-invocable: true
---
...
Khi `--remote` hoặc config remote tồn tại, kiểm tra server reachable:
...
1. Cập nhật state: `current_phase = step.id`, `hitl_pending = null` → sync remote nếu bật.
...
6. **Sync remote** nếu `syncAfterStep` bật và `runnerMode=local` (Luồng B).
