...
- **Knowledge mode** (rail icon): quản lý entry Markdown tại `.dev-team-agent/knowledge/` — tạo/sửa, upload `.md`/`.txt`, gắn tags, lọc theo scope (`project`/`system`). Pipeline Editor step config hỗ trợ `knowledge_inputs` (id entry) cho orchestrator inject.
...
| `GET /api/knowledge` | List knowledge entries (`scope`, `tags`, `q`) |
| `GET /api/knowledge?id=` | Đọc một entry |
| `POST /api/knowledge` | Tạo entry (JSON) |
| `PUT /api/knowledge?id=` | Cập nhật entry |
| `DELETE /api/knowledge?id=` | Xóa entry |
| `POST /api/knowledge/upload` | Upload file multipart |
| `GET /api/knowledge/tags` | Danh sách tags + count |
